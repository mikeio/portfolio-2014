<!DOCTYPE html>

<!--[if IE 8]>    <html class="no-js lt-ie9" lang="en"> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js" lang="en"> <!--<![endif]-->

<!-- Start Header Section -->
<?php include 'head.php'; ?>
<!-- End Header Section -->

<body>
	<!-- Start Navigation Section -->
	<?php include 'nav.php'; ?>
	<!-- End Navigation Section -->

	<!-- Start Header Section -->
	<div class="header header_alt">

		<div class="header-text">
			<h2>Tedo Stone</h2>
			<h6 id="sub_header">Psychedelic Rocker</h6>
			<hr>
		</div>
		
	</div>
	<!-- End Header Section -->


	<!-- Start Content Section -->

	<div class="content content_alt">

		<div class="bg_gradient">

			<div class="row">
	
				<div class="large-3 push-9 columns">

					<div class="sidebar">
	
						<h6 id="inverse">About:</h6>
						<p>Tedo Stone’s music never ceases to fuse throw-back sounds (primarily of the 1960’s) with a keen sense of contemporary resonance. In fact, his seemingly effortless eclecticism perhaps stands out most, as he places psychedelic rock, soul, blues, country, and even sometimes electronic music all under one roof.</p>
						<hr>
						<h6>Services:</h6>
						<h4>Design</h4>
						<h6>Date:</h6>
						<h4>2013</h4>
		
						<a href="http://www.tedostone.com" class="btn-medium btn-sidebar">Launch Site</a>

					</div>
	
				</div>
	
				<div class="large-9 pull-3 columns">
					<a href="img/gallery-tedostone01.jpg" class="fancybox wow fadeInUp" data-wow-offset="100" rel="gallery"><img src="img/gallery-tedostone01.jpg" class="portfolio-img" alt="Tedo Stone"></a>
					<a href="img/gallery-tedostone02.jpg" class="fancybox wow fadeInUp" data-wow-offset="100" rel="gallery"><img src="img/gallery-tedostone02.jpg" class="portfolio-img-last" alt="Tedo Stone"></a>
				</div>
	
			</div>

		</div>

		<!-- Start Footer Section -->
		<?php include 'footer.php'; ?>
		<!-- End Footer Section -->
	
	</div>

	<!-- End Content Section -->

  <script>
  document.write('<script src=js/vendor/' +
  ('__proto__' in {} ? 'zepto' : 'jquery') +
  '.js><\/script>')
  </script>

  <script src="js/foundation.min.js"></script>

  <script>
    $(document).foundation();
  </script>

  <script src="js/effects-alt.js"></script>

</body>

</html>